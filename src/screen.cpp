...
#define OLED_SET_HORIZ_RIGHT_SCROLL _u(0x26)
#define OLED_SET_HORIZ_LEFT_SCROLL _u(0x27)
...
void init_screen(uint8_t scl_pin = PICO_DEFAULT_I2C_SCL_PIN, uint8_t sda_pin = PICO_DEFAULT_I2C_SDA_PIN) {
...
void draw() {
    ...
    struct render_area frame_area = {.start_col =  0, .end_col =  OLED_WIDTH - 1, .start_page =  0, .end_page =
    OLED_NUM_PAGES -
    1};
    ...
    struct render_area area = {.start_col =  0, .end_col =  IMG_WIDTH - 1, .start_page =  0, .end_page =
    OLED_NUM_PAGES - 1};
